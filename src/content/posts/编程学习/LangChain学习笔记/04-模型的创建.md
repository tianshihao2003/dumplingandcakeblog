...
tags:
  - LangChain
  - AI
image: https://img.tsh520.cn/file/blog/post-covers/langchain-04-model-creation.webp
order: 4
---
...
1. Model I/O 三件套：输入提示（____）、调用模型（____）、输出解析（____）；LangChain 本身不提供 ____，它只是个"工具箱"；创建模型要看清三个角度：调用谁家的 ____、重要参数写在 ____（还是硬编码）、模型 ____ 在哪（在线还是____）
2. 每个平台配置都需要三个要素：____、____、____；`load_dotenv(override=____)` 里 True 表示 .env 的值 ____ 已有环境变量；`.env` 里不要加 ____、必须加进 ____；把密钥直接写进代码只适用于 ____
3. `ChatDeepSeek` 里 API 地址参数叫 ____（不是 `base_url`）；不传 `api_key` 时会自动从环境变量 ____ 读取；模型名参数既接受 `model` 也接受 ____
4. 智谱专用类在 ____ 包里（还要装 ____ 依赖）；千问专用类底层走的是 ____（不是 OpenAI 兼容接口），所以不要添加 ____ 环境变量，否则会报连接被强制关闭的 ____
5. 大多数平台都支持 ____ API 接口规范，所以基本都能用 `____` 集成；OpenRouter 有专用集成类 ____，它的模型名要带 ____；CloseAI 没有专用集成，只能走 ____ 或统一接口
6. `init_chat_model` 的模型标识写法是 `"____:____"`，它和 ____ 参数等价；两者都不写时按模型名前缀推断（`claude...` → ____、`deepseek...` → ____）；`qwen-plus` 会抛 ____ 错，必须显式指定 provider
7. 常用初始化参数：`temperature` 默认 ____、取值范围 0.0–2.0（写代码/数据提取建议用 ____ 区间）；`max_tokens` 限制的是 ____ 的 token 数；`timeout` 单位是 ____；`max_retries` 默认 ____
8. 想看某模型类的完整参数清单，要看类属性 ____（`ChatDeepSeek` 有 ____ 个、`ChatOpenAI` 有 ____ 个，清单里还能看到模型能力画像字段 ____）；参数分四大类：____、____、____、____；把变量名当参数名写时，多余参数会被塞进 ____（只给一条 ____ 警告），最后抛 ____ 错
9. 透传参数：OpenAI 兼容文档里的字段（如用于函数调用的 ____）放 ____，厂商自己扩展的字段（如 DeepSeek 的 ____）放 ____；Token 是模型用 ____ 切分出的最小语义单元，中文 1 token ≈ ____ 个汉字、英文 1 token ≈ ____ 个字符
10. 本地部署框架 ____ 常用命令：____ 下载模型、____ 列出本机已有模型、____ 查看正在运行的模型进程；LangChain 里用 ____ 类调用，或用统一接口把 provider 设为 "____"

> [!TIP]- 填空答案（做完再点开）
> 1. Format / Predict / Parse / 任何 LLM；API / 配置文件还是硬编码 / 部署（在线还是本地）　2. 模型名 / api-key / base-url；True / 覆盖；引号 / `.gitignore` / 临时测试　3. `api_base` / `DEEPSEEK_API_KEY` / `model_name`　4. `langchain-community` / `pyjwt`；专用 SDK / `DASHSCOPE_BASE_URL` / `ConnectionError`（ConnectionResetError 10054）　5. OpenAI / `ChatOpenAI`；`ChatOpenRouter` / 厂商前缀（如 `deepseek/…`）；`ChatOpenAI`　6. provider / model_name；`model_provider`；anthropic / deepseek；`ValueError`（Unable to infer model provider）　7. 0.7 / 0.0–0.3；输出 / 秒 / 6　8. `model_fields` / 56 / 54 / `profile`；客户端与连接参数、模型推理参数、LangChain 框架通用参数、高级与特定扩展参数；`model_kwargs` / `UserWarning` / `OpenAIError: Missing credentials`　9. `tools` / `model_kwargs`；`thinking` / `extra_body`；分词器（Tokenizer）/ 1–1.8 / 3–4　10. Ollama / `ollama pull` / `ollama list` / `ollama ps`；`ChatOllama` / `ollama`

### 二、裸写题

- [ ] **2-1 用厂商专用类创建 DeepSeek 模型**
  从 .env 读密钥，用 DeepSeek 的厂商专用类创建模型（模型名 `deepseek-v4-flash`）：先显式传三个值跑一次，再不传密钥跑一次（看它能不能自己从环境变量里读到），最后换成它的另一个等价模型名参数再跑一次，每次打印回复文本。

  > [!TIP]- 提示（先自己想，实在想不出再点开）
  > **一级 · 思路**：先 load_dotenv，再用厂商类实例化，最后 invoke；本例的关键是记住这个类的地址参数名和 OpenAI 那套不一样
  > **二级 · 方法**：`ChatDeepSeek(api_key=..., api_base=..., model=...)`，不传 `api_key` 时会读环境变量 `DEEPSEEK_API_KEY`
  > **三级 · 骨架**：`llm = ChatDeepSeek(api_base=os.getenv("DEEPSEEK_BASE_URL"), model_name="deepseek-v4-flash")`

- [ ] **2-2 用兼容方式创建（换平台只改两处）**
  挑一个 LangChain 没有提供专用类的平台（如 CloseAI），用兼容类创建模型并打印一次回复；再把它写成函数，演示"同一个模型在 DeepSeek 官方 / CloseAI / OpenRouter 之间切换"，只改密钥、地址和模型名三个值。

  > [!TIP]- 提示
  > **一级 · 思路**：兼容写法只要 3 样东西——密钥、地址、模型名，代码结构完全不用动
  > **二级 · 方法**：`ChatOpenAI(api_key=..., base_url=..., model=...)`（可用别名 `openai_api_base`）
  > **三级 · 骨架**：`def build_llm(key_env, url_env, model_name): return ChatOpenAI(...)`，OpenRouter 的模型名要带厂商前缀

- [ ] **2-3 用统一接口创建并切换**
  用统一接口创建 DeepSeek 模型，再写一个"只改模型字符串就能换模型"的函数；然后故意不指明提供商去初始化一个 `qwen-plus`，看报什么错、正确的写法是什么。

  > [!TIP]- 提示
  > **一级 · 思路**：统一接口的意义是"换模型/换平台不改代码结构"；provider 可以写在参数里，也可以写成模型字符串的前缀
  > **二级 · 方法**：`init_chat_model(model="provider:model_name", model_provider=..., base_url=..., api_key=...)`
  > **三级 · 骨架**：两个位置都不写 provider 时 LangChain 会按前缀猜，猜不出来抛 `ValueError`；这时要显式写 `model_provider="openai"` 并配上百炼的兼容地址

- [ ] **2-4 透传框架没有定义的参数**
  ① 把一段"模型参数清单里查不到、但 OpenAI 兼容文档里有"的字段（函数调用用的工具列表）透传进去，问一句天气，打印返回里的工具调用和结束原因；
  ② 把 DeepSeek 自己扩展的"思考开关"透传进去，分别用启用/禁用各问一句，对比两次的思考内容和 reasoning token。

  > [!TIP]- 提示
  > **一级 · 思路**：OpenAI 兼容字段和厂商扩展字段走的是两条不同的透传通道
  > **二级 · 方法**：`model_kwargs={"tools": [...]}`；`extra_body={"thinking": {"type": "enabled"}}`；想看思考内容要让它走 DeepSeek 自己的类
  > **三级 · 骨架**：`init_chat_model(..., model_kwargs={...})` / `init_chat_model(model="deepseek:deepseek-v4-flash", extra_body={...})`，读 `response.tool_calls`、`response.additional_kwargs`、`response.usage_metadata`

### 三、综合题

- [ ] **3-1 模型自检小工具**
  写一个"创建前先自检、调用后能算账"的小工具：
  1. 从 .env 读配置，用统一接口创建一个 OpenAI 兼容的模型（把随机性调到"数学计算、数据提取"那档区间）；
  2. 打印这个实例的完整参数名清单和总数；
  3. 按四大分类各挑出几个参数名打印（连接类 / 推理类 / 框架通用 / 高级扩展），挑不出来就说明名字记错了；
  4. 给它透传"思考开关"（先启用）问一句 `1 + 1 = ？`，打印回答；
  5. 打印这次调用的输入 / 输出 / 总 token，并按"中文 1 token ≈ 1–1.8 个汉字"估算输出对应多少汉字；
  6. 把开关换成禁用再调一次，对比两次的思考内容与 reasoning token。

  > [!TIP]- 提示（先自己想，实在想不出再点开）
  > **一级 · 思路**：创建（统一接口）→ 自检（参数清单）→ 归类（四大分类）→ 透传（思考开关）→ 调用 → 算账（token）
  > **二级 · 方法**：`type(llm).model_fields.keys()` 拿清单；`extra_body={"thinking": {...}}` 控开关；`resp.usage_metadata["output_tokens"]` 算账单
  > **三级 · 骨架**：`def ask(thinking): return model.invoke("1 + 1 = ？")`；两次结果对比 `additional_kwargs.get("reasoning_content")` 和 `usage_metadata["output_token_details"]`
...
> from langchain_deepseek import ChatDeepSeek
>
> # 写法 1：三个值都显式传（专用类的地址参数叫 api_base，不是 base_url）
> llm = ChatDeepSeek(
>     api_key=DEEPSEEK_API_KEY,
>     api_base=DEEPSEEK_BASE_URL,
>     model="deepseek-v4-flash",
> )
> print("写法1 →", llm.invoke("请介绍一下你自己").content)
>
> # 写法 2：api_key 不传，会自动读环境变量 DEEPSEEK_API_KEY
> llm2 = ChatDeepSeek(
>     api_base=DEEPSEEK_BASE_URL,
>     model="deepseek-v4-flash",
> )
> print("写法2 →", llm2.invoke("用一句话介绍下你自己").content)
>
> # 模型名参数既接受 model，也接受 model_name（两者等价）
> llm3 = ChatDeepSeek(
>     api_key=DEEPSEEK_API_KEY,
>     api_base=DEEPSEEK_BASE_URL,
>     model_name="deepseek-v4-flash",
> )
> print("model_name 别名 →", llm3.invoke("1 + 1 = ？").content)
>
>
> # ========== 2-2 兼容方式（换平台只改两处） ==========
> from langchain_openai import ChatOpenAI
>
> def build_compat(key_env: str, url_env: str, model_name: str):
>     """一个兼容类打通所有 OpenAI 兼容平台：换平台只改 base_url 和 model"""
>     return ChatOpenAI(
>         api_key=os.getenv(key_env),
>         base_url=os.getenv(url_env),      # 兼容类用 base_url（别名 openai_api_base）
>         model=model_name,
>     )
>
> # ① CloseAI：LangChain 没有给它专用类，只能走兼容接口
> closeai = build_compat("CLOSEAI_API_KEY", "CLOSEAI_BASE_URL", "deepseek-v4-flash")
> print("[CloseAI]", closeai.invoke("欧盟都有哪些国家").content)
>
> # ② DeepSeek 官方：同一套代码，只换了「密钥变量 + 地址变量 + 模型名」
> deepseek = build_compat("DEEPSEEK_API_KEY", "DEEPSEEK_BASE_URL", "deepseek-v4-flash")
> print("[DeepSeek]", deepseek.invoke("1 + 1 = ？").content)
>
> # ③ OpenRouter：模型名要带厂商前缀（也可以用它自己的专用集成，这里演示兼容写法）
> openrouter = build_compat("OPENROUTER_API_KEY", "OPENROUTER_API_BASE", "deepseek/deepseek-v4-flash")
> print("[OpenRouter]", openrouter.invoke("一句话介绍下你自己").content)
>
>
> # ========== 2-3 统一接口 + 切换模型 ==========
> def build_llm(model_name: str, provider: str = "openai"):
>     """只改模型字符串就换模型"""
>     return init_chat_model(
>         model=model_name,
>         model_provider=provider,          # 不写就按模型名前缀猜（猜不出来会报 ValueError）
>         base_url=DEEPSEEK_BASE_URL,       # 走 OpenAI 兼容协议时用 base_url
>         api_key=DEEPSEEK_API_KEY,
>     )
>
> for name in ["deepseek-v4-flash", "deepseek-v4-pro"]:
>     print(name, "→", build_llm(name).invoke("用一句话介绍你自己").content)
>
> # 等价写法：把 provider 写在模型字符串里（前缀 + 冒号），效果和 model_provider 一样
> same = init_chat_model(
>     model="deepseek:deepseek-v4-flash",
>     api_key=DEEPSEEK_API_KEY,
>     api_base=DEEPSEEK_BASE_URL,     # provider 解析成 deepseek 后，它认的是自己的 api_base
> )
> print("model='deepseek:deepseek-v4-flash' →", same.invoke("1 + 1 = ？").content)
>
> # 两个地方都不写 provider 时，LangChain 按前缀猜；猜不出来就是这种报错
> try:
>     init_chat_model(model="qwen-plus")
> except ValueError as e:
>     print("推断失败 →", e)
>     # 正确姿势：显式 model_provider="openai" + 百炼的 OpenAI 兼容地址
>
> # 本地部署的模型也是同一套代码，只换 provider
> # （本机没装 Ollama，这段只做语法核对；真跑之前要先 `ollama serve`）
> # local = init_chat_model(model="deepseek-r1:1.5b", model_provider="ollama",
> #                         base_url="http://localhost:11434")
> # print(local.invoke("你好").content)
>
>
> # ========== 2-4 透传框架没有定义的参数 ==========
> # ① model_kwargs：OpenAI 兼容文档里有、但 LangChain 没有单列出来的字段（比如 tools）
...
>     },
> )
> r = model_with_tools.invoke("你好，今天北京的天气如何")
> print("tool_calls:", r.tool_calls)
> print("finish_reason:", r.response_metadata.get("finish_reason"))     # tool_calls
> # 注意：实际开发中要用专门的工具调用接口，这里只是演示 model_kwargs 的作用
>
> # ② extra_body：厂商在 OpenAI 协议之外自己扩展的字段（DeepSeek 的 thinking）
> for mode in ("enabled", "disabled"):
>     llm = init_chat_model(
>         model="deepseek:deepseek-v4-flash",   # 用 DeepSeek 自己的类，才看得到 reasoning_content
>         api_key=DEEPSEEK_API_KEY,
>         api_base=DEEPSEEK_BASE_URL,
>         extra_body={"thinking": {"type": mode}},
>     )
>     resp = llm.invoke("你好，一句话回答")
>     print(f"\nthinking={mode} 回答：{resp.content}")
>     print("   reasoning_content:", repr(resp.additional_kwargs.get("reasoning_content")))
>     print("   reasoning tokens:", resp.usage_metadata.get("output_token_details"))
> # 观察：enabled 时有思考内容、reasoning token 非 0；disabled 时两样都没有
>
>
> # ========== 3-1 综合：模型自检小工具 ==========
> # 第 1 步：创建模型（temperature 取「要一致性、准确性」那档：0.0–0.3）
> llm = init_chat_model(
>     model="deepseek-v4-flash",
>     model_provider="openai",
>     base_url=DEEPSEEK_BASE_URL,
...
> )
>
> # 第 2 步：打印完整参数清单 + 总数（参数一部分自己定义、一部分继承自父类）
> params = list(type(llm).model_fields.keys())
> print("参数总数:", len(params), "（类名", type(llm).__name__ + "）")
>
> # 第 3 步：从清单里按四大分类各挑几个（挑出来才说明这些名字确实存在）
> groups = {
>     "① 连接类": ["openai_api_key", "openai_api_base", "request_timeout", "max_retries",
>                  "openai_proxy", "default_headers", "default_query", "http_client"],
>     "② 推理类": ["model_name", "temperature", "max_tokens", "top_p", "stop", "n",
>                  "presence_penalty", "frequency_penalty", "reasoning_effort", "store"],
>     "③ 框架通用": ["name", "verbose", "callbacks", "tags", "metadata", "cache",
>                    "rate_limiter", "custom_get_token_ids"],
>     "④ 高级扩展": ["model_kwargs", "extra_body", "client", "async_client", "root_client",
>                    "service_tier", "disable_streaming", "include_response_headers"],
> }
> for label, names in groups.items():
>     print(label, "→", [n for n in names if n in params])
> # 提示：换成 ChatDeepSeek 会多出 api_key、api_base 两个自己的参数（56 vs 54）
>
> # 第 4 步：透传「思考」开关，各调用一次
> def ask(thinking: str):
>     m = init_chat_model(
>         model="deepseek:deepseek-v4-flash",
>         api_key=DEEPSEEK_API_KEY,
>         api_base=DEEPSEEK_BASE_URL,
...
> r_off = ask("disabled")
>
> # 第 5 步：回答 + 账单（token 既是长度单位也是账单单位）
> usage = r_on.usage_metadata
> print("\n回答:", r_on.content)
> print(f"输入 / 输出 / 总计 token: {usage['input_tokens']} / {usage['output_tokens']} / {usage['total_tokens']}")
> print("按「中文 1 token ≈ 1–1.8 个汉字」估算，输出的 {} 个 token 大约对应 {}–{} 个汉字".format(
>     usage["output_tokens"], usage["output_tokens"], round(usage["output_tokens"] * 1.8)))
>
> # 第 6 步：对比两次的思考开销
> print("\n思考内容（enabled）:", (r_on.additional_kwargs.get("reasoning_content") or "无")[:40])
> print("思考内容（disabled）:", r_off.additional_kwargs.get("reasoning_content") or "无")
> print("reasoning token：enabled =", r_on.usage_metadata.get("output_token_details"),
>       "｜disabled =", r_off.usage_metadata.get("output_token_details"))
> ```